...
export default function ObserverStudentList() {
  ...
  return <PageContainer title="负责学生" description="列表范围仅包含由管理员明确分配给你的学生。" extra={<Input.Search value={search} onChange={(e) => setSearch(e.target.value)} onSearch={() => load()} placeholder="姓名或账号" allowClear />}><Card className="content-card"><AsyncPageState loading={loading} error={error} onRetry={() => load()} empty={!data.items.length} emptyText="暂无明确分配的学生"><Table rowKey="id" columns={columns} dataSource={data.items} pagination={{ current: data.pagination.page, total: data.pagination.total, pageSize: 20, onChange: load }} /></AsyncPageState></Card></PageContainer>;
}
