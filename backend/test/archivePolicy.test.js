...
test('导师学生列表与档案树、详情三入口一致', async () => {
  ...
  assert.deepEqual(listB.students.map((s) => s.id), [], '仅授课而非创建课程的导师不能浏览学生');

  assert.equal((await authed(tokenB, 'GET', '/api/students/4', null)).status, 403, '仅授课课程学生详情应403');
  assert.equal((await authed(tokenB, 'GET', '/api/archives/generate?student_id=4', null)).status, 403, '仅授课课程学生档案应403');
...
test('成长记录：教师限本校，导师限相关学生，管理员可写', async () => {
