...
| 学术导师 `academic_mentor` | 唯一课程执行导师：管理自己创建或授课的课程、课时、知识卡片、小练习、任务和选课；评审作品与学习报告；管理课程范围内的成长档案 |
...
- 学生列表、学生详情、档案树、单人档案和按学校/班级批量导出，均限定为执行导师自己创建的课程中有报名记录的学生；搜索或手动填写学生 ID 不能扩大范围。导师也不能为无关学生添加成长记录。
- 按方案中的报名关系判断，保留已移除报名和已归档课程的历史关联；学生停用或归档后，相关导师仍可查看其历史档案。多个课程关联同一学生不会导致列表重复。此处限制的是学生范围，不是将获准查阅的学生档案拆分为单门课程。
- 在自己课程的“导入学生”候选查询中，导师仍可搜索全平台符合条件的学生。没有相关学生时，普通列表和档案树为空，不会退回全平台数据。
- 新增 `studentPolicy.js` 统一导师关系判断，管理员、教师本校及学生本人原有访问规则保持不变。作品独立接口、正式评价和教师课程关系仍留待后续整改。本批无数据库结构变更，不需要初始化或重置数据库。

### 执行导师课程管理范围（方案第 8 条）

- 管理员可管理所有课程；执行导师只能管理自己创建的课程。课程编辑、发布/撤回、删除、课时和任务创建、资源与回放上传、回放修改/删除、学生导入及候选查询均执行这一规则。导师管理自己课程时，仍可在全平台搜索符合条件的候选学生。
- 课程列表、详情页按每门课程的管理权限显示操作入口；其他导师或管理员创建的课程显示为只读，直接访问编辑地址会返回详情。资源和视频在接收上传文件前检查权限。
- 本批建立 `backend/policies/coursePolicy.js`，不调整课程读取范围、教师原有选课规则、学生与档案读取范围；这些仍按后续整改条目推进。多人共同负责课程的 `course_managers` 尚未实现。
...
- 第 6 条账号写权限及第 7 条删除数据保护已实施；导师学生/档案读取范围见第 9 条进度，空测试账号身份判定及教师跨模块读取范围仍待后续整改。
